...
resource "aws_instance" "host" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      tar --exclude="./build/infrastructure/" -czvf ./tmp/archive.tar.gz ./app/Dockerfile ./app/zend/ ./app/laravel.composer.lock/ ./bin ./build ./config ./tmp/results/${var.result_root_dir} .dockerignore Dockerfile
EOF
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "export INFRA_INSTANCE_TYPE=\"${var.instance_type}\"",
      "export INFRA_ARCHITECTURE=\"${var.image_architecture}\"",
      "export INFRA_DEDICATED_INSTANCE=\"${var.use_dedicated_instance}\"",
      "export INFRA_DISABLE_DEEPER_C_STATES=\"${var.disable_deeper_c_states}\"",
      "export INFRA_DISABLE_TURBO_BOOST=\"${var.disable_turbo_boost}\"",
      "export INFRA_DISABLE_HYPER_THREADING=\"${var.disable_hyper_threading}\"",
      "export INFRA_ENVIRONMENT=\"${var.environment}\"",
      "export INFRA_RUNNER=\"${var.runner}\"",
      "export INFRA_MEASURE_INSTRUCTION_COUNT=\"${var.measure_instruction_count}\"",
      "export INFRA_DOCKER_REGISTRY=\"${var.docker_registry}\"",
      ...
      # Verify CPU and kernel settings
